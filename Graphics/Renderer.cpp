...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	nanogui::ref<nanogui::Window> nanoguiWindow_1 = gui_1->addWindow(Eigen::Vector2i(0, 0), "Camera Controls");
	...
	//My added code
	gui_1->addGroup("Rotation");
	gui_1->addVariable("Degree a", m_animation->a)->setSpinnable(true);
	gui_1->addButton("Rotate X", []() {
		double a = m_animation->a;
		m_animation->Xrotate_local(a);
	});
	gui_1->addVariable("Degree b", m_animation->b)->setSpinnable(true);
	gui_1->addButton("Rotate Y", []() {
		double b = m_animation->b;
		m_animation->Yrotate_world(b);
	});
	gui_1->addButton("Reset Animation", []() {
		m_animation->reset();
	});
	//My added code
...
void Renderer::draw_scene(Shader& shader)
{
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "main_object")
		{
			...
			glm::mat4 main_object_model_mat =  glm::mat4(1.0);
		...
		{
			// Draw plane
			glm::mat4 plane_model_mat =  glm::mat4(1.0);
		...
		{
			// Draw three axis
			glm::mat4 model_mat_x = glm::mat4(1.0);
			...
			glm::mat4 model_mat_y = glm::mat4(1.0);
			...
			glm::mat4 model_mat_z = glm::mat4(1.0);
